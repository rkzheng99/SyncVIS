...
from .modeling.syncvis_criterion import syncvisSetCriterion
from .modeling.syncvis_matcher import syncvisHungarianMatcher
from .modeling.transformer_decoder.syncvis import syncvis
import pdb

@META_ARCH_REGISTRY.register()
class syncvis(nn.Module):
    """
    ...
    def __init__(
        ...
        *,
        ...
        # syncvis
        syncvis_module: nn.Module,
        syncvis_criterion: nn.Module,
        num_frames: int,
        num_classes: int,
        is_multi_cls: bool,
        ...
        # syncvis hyper-parameters
        self.num_frames = num_frames
        self.num_classes = num_classes
        self.syncvis_module = syncvis_module
        self.syncvis_criterion = syncvis_criterion
        self.is_multi_cls = is_multi_cls
        self.apply_cls_thres = apply_cls_thres

        if freeze_detector:
            for name, p in self.named_parameters():
                if not "syncvis_module" in name:
    ...
    def from_config(cls, cfg):
        ...
        syncvis_deep_supervision = cfg.MODEL.syncvis.DEEP_SUPERVISION

        # loss weights
        class_weight = cfg.MODEL.MASK_FORMER.CLASS_WEIGHT
        dice_weight = cfg.MODEL.MASK_FORMER.DICE_WEIGHT
        mask_weight = cfg.MODEL.MASK_FORMER.MASK_WEIGHT
        sim_weight = cfg.MODEL.syncvis.SIM_WEIGHT
        ...
        criterion = SetCriterion(
            ...
            syncvis_last_layer_num=cfg.MODEL.syncvis.LAST_LAYER_NUM,
        )

        # syncvis
        num_classes = sem_seg_head.num_classes
        hidden_dim = cfg.MODEL.MASK_FORMER.HIDDEN_DIM
        syncvis_module = syncvis(cfg=cfg, in_channels=hidden_dim, aux_loss=syncvis_deep_supervision)

        # building criterion for syncvis inference
        syncvis_matcher = syncvisHungarianMatcher(
            cost_class=class_weight,
            cost_mask=mask_weight,
            cost_dice=dice_weight,
            num_points=cfg.MODEL.MASK_FORMER.TRAIN_NUM_POINTS,
        ...
        syncvis_weight_dict = {
            "loss_syncvis_ce": class_weight, "loss_syncvis_mask": mask_weight, "loss_syncvis_dice": dice_weight
        }
        if sim_weight > 0.0:
            syncvis_weight_dict["loss_syncvis_sim"] = sim_weight

        if syncvis_deep_supervision:
            syncvis_dec_layers = cfg.MODEL.syncvis.DEC_LAYERS
            aux_weight_dict = {}
            for i in range(syncvis_dec_layers - 1):
                aux_weight_dict.update({k + f"_{i}": v for k, v in syncvis_weight_dict.items()})
            syncvis_weight_dict.update(aux_weight_dict)
        syncvis_losses = ["syncvis_labels", "syncvis_masks"]
        if sim_weight > 0.0:
            syncvis_losses.append("fg_sim")

        syncvis_criterion = syncvisSetCriterion(
            num_classes, 
            matcher=syncvis_matcher, 
            weight_dict=syncvis_weight_dict,
            eos_coef=cfg.MODEL.syncvis.NO_OBJECT_WEIGHT,
            losses=syncvis_losses, 
            num_points=cfg.MODEL.MASK_FORMER.TRAIN_NUM_POINTS,
            oversample_ratio=cfg.MODEL.MASK_FORMER.OVERSAMPLE_RATIO,
            importance_sample_ratio=cfg.MODEL.MASK_FORMER.IMPORTANCE_SAMPLE_RATIO,
            sim_use_clip=cfg.MODEL.syncvis.SIM_USE_CLIP,
        )
        ...
        return {
            ...
            # syncvis
            "syncvis_module": syncvis_module,
            "syncvis_criterion": syncvis_criterion,
            "num_frames": cfg.INPUT.SAMPLING_FRAME_NUM,
            "num_classes": num_classes,
            "is_multi_cls": cfg.MODEL.syncvis.MULTI_CLS_ON,
            "apply_cls_thres": cfg.MODEL.syncvis.APPLY_CLS_THRES,
            "freeze_detector": cfg.MODEL.syncvis.FREEZE_DETECTOR,
            "test_run_chunk_size": cfg.MODEL.syncvis.TEST_RUN_CHUNK_SIZE,
            "test_interpolate_chunk_size": cfg.MODEL.syncvis.TEST_INTERPOLATE_CHUNK_SIZE,
    ...
    def train_model(self, batched_inputs):
        ...
        mask_features = self.syncvis_module.syncvis_mask_features(mask_features)
        ...
        syncvis_outputs = self.syncvis_module(frame_queries)
        syncvis_outputs["pred_masks"] = torch.einsum("lbqc,btchw->lbqthw", syncvis_outputs["pred_mask_embed"], mask_features)
        for out in syncvis_outputs["aux_outputs"]:
            out["pred_masks"] = torch.einsum("lbqc,btchw->lbqthw", out["pred_mask_embed"], mask_features)

        for i in range(len(idx_list)):
            for k in list(losses_list[i].keys()):
                if k in self.criterion.weight_dict:
                    losses[k+'_'+str(i)] = losses_list[i][k] * self.criterion.weight_dict[k] * weight_list[i] * 0.5
        ...
        for k in syncvis_loss_dict.keys():
            if k in syncvis_weight_dict:
                syncvis_loss_dict[k] *= syncvis_weight_dict[k]
        losses.update(syncvis_loss_dict)
        return losses
    ...
    def inference(self, batched_inputs):
        ...
        for i in range(math.ceil(num_frames / self.test_run_chunk_size)):
            ...
            _mask_features = self.syncvis_module.syncvis_mask_features(_mask_features)
        ...
        mask_cls = syncvis_outputs["pred_logits"][-1, 0]       # cQ, K+1
        mask_embed = syncvis_outputs["pred_mask_embed"][-1, 0] # cQ, C
