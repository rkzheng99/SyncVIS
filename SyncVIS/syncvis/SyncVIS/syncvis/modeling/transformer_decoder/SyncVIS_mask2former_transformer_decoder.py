...
class syncvisMultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.FFN_3 =  FFNLayer(d_model=hidden_dim, dim_feedforward=dim_feedforward, dropout=0.0, normalize_before=pre_norm,)
    ...
    def forward(self, x, mask_features, clip_mask_features, mask = None):
        ...
        if not self.training:
            ...
            for ii in range(len(idx_list)):
                ...
                outputs_class1, outputs_mask1, attn_mask1, frame_query1= self.forward_prediction_heads(output1_s, clip_mask_features, attn_mask_target_size=size_list[0])
                ...
                for i in range(self.num_layers):
                    ...
                    output_s_tmp = self.FFN_2(output_s)
                    output_s_cache, weights = self.multi_attn(output_s_tmp,ref_query,ref_query)
                    
                    output_s = (1 - lamda) * output_s + lamda * output_s_cache
                    outputs_class, outputs_mask1, attn_mask1, frame_query1 = self.forward_prediction_heads(output_s, mask_features_s, attn_mask_target_size=size_list[(i + 1) % self.num_feature_levels])
                    ...
                    ref_query_s = torch.cat([output1[:, i, :][select_indices1[i, :], :] for i in range(5)], dim=1)
                    output1_s_tmp = self.FFN_3(output1_s)                                            
                    output1_s_cache, weights = self.multi_attn(output1_s_tmp,ref_query_s,ref_query_s)
                    output1_s = (1 - lamda) * output1_s + lamda * output1_s_cache
                    outputs_class, outputs_mask, attn_mask, frame_query = self.forward_prediction_heads(output_s, mask_features, attn_mask_target_size=size_list[0])
                    frame_queries.append(frame_query)
                    predictions_class.append(outputs_class)
                    predictions_mask.append(outputs_mask)
            ...
            for i in range(self.num_layers):
                ...
                out_logits,logits_indices = scores.topk(k=10,dim=2) #,100,k<40
                out_logits = torch.sum(out_logits,dim=2)
                select_logits, select_indices = out_logits.topk(k=10,dim=1,largest=True)  #*10

                select_indices = select_indices.unsqueeze(0).permute(1, 0)

                ref_query = torch.cat([output1[:, i, :][select_indices[i, :], :] for i in range(5)], dim=1)
                
                output_cache, weights = self.multi_attn(output,ref_query,ref_query)
